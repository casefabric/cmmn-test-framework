...
export default class TestArchiveCase extends TestCase {
  ...
  async test(definition: Definitions) {
    ...
    this.reader.question(`Press enter to start test loop on ${definition} with case hierarchy\n ${caseHierarchy}`);
    ...
    while (true) {
      const line = this.reader.question("Type p and press Enter to print event hierarchy. Press enter to archive the case ");
      if (line.indexOf('p') >= 0) {
    ...
    console.log("Events: " + caseHierarchy.printEvents());
    this.reader.question("Case is fully archived; press enter to continue assertion tests");
