...
import CaseTeamGroup, { GroupRoleMapping } from '../../../src/cmmn/team/caseteamgroup';
...
import ConsentGroupEvents from '../../../src/service/storage/consentgroupevents';
import StorageService from '../../../src/service/storage/storageservice';
import TenantEvents from '../../../src/service/storage/tenantevents';
...
export default class TestDeleteTenantWithContent extends TestCase {
  ...
  async run() {
    ...
    this.reader.question(`Press enter to delete tenant ${tenant} with ${this.tenantCases.length} case instances`);
  ...
  async createTenantCases() {
    ...
    const input = this.reader.question(
    ...
    if (input) {
      ...
      if (array.length > 1) {
        file = array[1];
        this.reader.question("Confirm that we create " + count + " cases of type " + file)
      }

      const definition = new Definition(file);
      await definition.deploy(user, tenant);
