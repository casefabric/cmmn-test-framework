...
export default class CommandLineParser {
    isNPM: boolean = true;;
    configArguments: Array<string> = [];


    constructor() {
        this.isNPM = process.argv.length > 0 && process.argv[0].toLowerCase() === 'npm'
        this.configArguments = this.isNPM ? process.argv.slice(3) : process.argv.slice(2);
        this.parseTimeout();
        this.parseLogging();
