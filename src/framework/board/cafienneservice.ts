...
import QueryFilter, { extendURL } from '@cafienne/typescript-client/service/queryfilter';
import CafienneResponse from '@cafienne/typescript-client/service/response';
import OriginalCafienneService from '@cafienne/typescript-client/service/cafienneservice';
...
export default class CafienneService {
    ...
    static async fetch(user: User | undefined, url: string, method: string, headers?: any, body?: string): Promise<CafienneResponse> {
        headers = getHeaders(user, headers);
        ...
        const myCallNumber = OriginalCafienneService.calls++;
...
function getHeaders(user: User | undefined, from: any = {}) {
    ...
    Object.entries(from).forEach(([headerName, headerValue]) => {
