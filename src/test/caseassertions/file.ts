 ...
 */
export default async function assertCaseFileContent(user: User, caseId: Case | string, path: string, expectedContent: any, log: boolean = false, trace: Trace = new Trace()) {
    await PollUntilSuccess(async function() {
            ...
            if (!Comparison.sameJSON(actualCaseFileItem, expectedContent, log)) {
                throw new AsyncError(trace, `Case File [${path}] is expected to match: ${JSON.stringify(expectedContent, undefined, 2)}\nActual: ${JSON.stringify(actualCaseFileItem, undefined, 2)}`);
            }
