...
//                throw new Error(`Ping service has not been invoked within ${millis} milliseconds. Aborting test`);
    ...
    async start() {
        ...
        this.express.get('/getListWebService', (req, res, next) => {
            const KEY_LIST: String[] = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'];
            let jsonList: string = '{"_2" :[';
        ...
        this.express.get('/details/:detailsId', (req, res, next) => {
            const detailsKey = req.params['detailsId'];
            const detailsBody: string = '{ "_2" : { "description": "details of ' + detailsKey + '", "id":"' + detailsKey + '"}}';
            res.writeHead(200, { 'Content-Type': 'text/json' });
            res.write(detailsBody);
            res.end();
        });
        return promise;
    }
